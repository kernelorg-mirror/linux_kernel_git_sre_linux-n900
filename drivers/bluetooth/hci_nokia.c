...
static irqreturn_t wakeup_handler(int irq, void *data)
{
	...
	dev_dbg(dev, "wakeup received: %d -> %d\n", btdev->wake_state,
		wake_state);
...
{
	struct nokia_bt_dev *btdev = serdev_device_get_drvdata(hu->serdev);
	...
	dev_dbg(dev, "Nokia H4+ protocol initialized\n");
...
{
	struct nokia_bt_dev *btdev = hu->priv;
	struct device *serialdev = &btdev->serdev->dev;
	...
	if (btdev->initialized && btdev->tx_enabled != !!result) {
		if (result) {
			...
			gpiod_set_value_cansleep(btdev->wakeup_bt, 0);
			if (!btdev->wake_state)
				serdev_device_set_flow_control(btdev->serdev,
							       false);
		}
