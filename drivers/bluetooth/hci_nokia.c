				 ...
				 int timeout_ms)
{
	...
	dev_dbg(dev, "wait for cts... timeout!\n");
	return -ETIMEDOUT;
}
...
static irqreturn_t wakeup_handler(int irq, void *data)
{
	...
	if (wake_state) {
		dev_dbg(dev, "wakeup RX=1\n");
		pm_runtime_get(dev);
	} else {
		dev_dbg(dev, "wakeup RX=0\n");
		pm_runtime_put(dev);
	}
...
static int nokia_reset(struct hci_uart *hu)
{
	...
	/* init uart */
	serdev_device_set_flow_control(btdev->serdev, false);
	...
	if (err < 0) {
		dev_err(dev, "CTS not received: %d\n", err);
		return err;
	}

	serdev_device_set_flow_control(btdev->serdev, true);
...
static int nokia_send_alive_packet(struct hci_uart *hu)
{
	...
	dev_dbg(dev, "Sending alive packet...\n");
...
static int nokia_send_negotiation(struct hci_uart *hu)
{
	...
	dev_dbg(dev, "Sending negotiation...\n");
...
static int nokia_setup(struct hci_uart *hu)
{
	...
	dev_dbg(dev, "Nokia H4+ protocol setup...\n");
	...
	if (err < 0) {
		dev_err(dev, "Negotiation failed: %d\n", err);
		goto out;
	}
	...
	if (err < 0) {
		dev_err(dev, "Alive check failed: %d\n", err);
		goto out;
	}
	...
	if (err < 0) {
		dev_err(dev, "Could not setup FW: %d\n", err);
		goto out;
	}
	...
	if (btdev->man_id == NOKIA_ID_BCM2048) {
		...
		/* FIXME: "btmgmt --index 0 public-addr 00:11:22:33:44:55" => "Set Public Address for hci0 failed with status 0x11 (Invalid Index)" */
		//set_bit(HCI_QUIRK_INVALID_BDADDR, &hu->hdev->quirks);
		dev_dbg(dev, "bcm2048 has invalid bluetooth address!\n");
	}

	dev_dbg(dev, "Nokia H4+ protocol setup done!\n");
...
static int nokia_open(struct hci_uart *hu)
{
	...
	dev_dbg(dev, "Nokia H4+ protocol open\n");
					 ...
					 struct sk_buff *skb)
{
	...
	if (evt->ack != NOKIA_NEG_ACK) {
		dev_err(dev, "Could not negotiate hci_nokia settings\n");
	...
	dev_dbg(dev, "NOKIA negotiation:\n");
	dev_dbg(dev, "\tbaudrate = %u\n", evt->baud);
	dev_dbg(dev, "\tsystem clock = %u\n", evt->sys_clk);
	dev_dbg(dev, "\tmanufacturer id = %u\n", evt->man_id);
	dev_dbg(dev, "\tversion id = %u\n", evt->ver_id);
...
static int nokia_recv_alive_packet(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (pkt->mid != NOKIA_ALIVE_RESP) {
		dev_err(dev, "Invalid alive response: 0x%02x!\n",
			pkt->mid);
		btdev->init_error = -EINVAL;
		goto finish_alive;
	}

	dev_dbg(dev, "Received alive packet!\n");
...
static struct sk_buff *nokia_dequeue(struct hci_uart *hu)
{
	...
	if (result) {
		dev_dbg(dev, "wakeup TX=1");
		pm_runtime_get_sync(dev);
		gpiod_set_value_cansleep(btdev->wakeup_bt, 1);
	} else {
		dev_dbg(dev, "wakeup TX=0");
		serdev_device_wait_until_sent(btdev->serdev, msecs_to_jiffies(1000)); /* FIXME: may hang without timeout */
		gpiod_set_value_cansleep(btdev->wakeup_bt, 0);
		pm_runtime_put(dev);
	}
...
static int nokia_bluetooth_serdev_probe(struct serdev_device *serdev)
{
	...
	btdev->reset = devm_gpiod_get(dev, "reset", GPIOD_OUT_LOW);
	if (IS_ERR(btdev->reset)) {
		err = PTR_ERR(btdev->reset);
		dev_err(dev, "could not get reset gpio: %d\n", err);
		return err;
	}
	...
	if (IS_ERR(btdev->wakeup_host)) {
		...
		dev_err(dev, "could not get host wakeup gpio: %d\n", err);
		return err;
	}
	...
	err = devm_request_threaded_irq(dev, btdev->wake_irq, NULL,
		...
		"wakeup", btdev);
	if (err) {
		dev_err(dev, "could request wakeup irq: %d\n", err);
		return err;
	}
	...
	if (IS_ERR(btdev->wakeup_bt)) {
		...
		dev_err(dev, "could not get BT wakeup gpio: %d\n", err);
		return err;
	}
	...
	if (IS_ERR(sysclk)) {
		err = PTR_ERR(sysclk);
		dev_err(dev, "could not get sysclk: %d\n", err);
		return err;
	}
	...
	dev_dbg(dev, "sysclk speed: %ld kHz\n", btdev->sysclk_speed / 1000);
	...
	dev_dbg(dev, "Nokia H4+ driver probed!\n");
